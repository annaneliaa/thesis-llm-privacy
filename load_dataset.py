...
if __name__ == "__main__":
  ...
  # these are the europarl files in english and dutch
  file_name = "europarl-v7.nl-en"
  languages = ["en", "nl"]

  lang = "en"
  split = sys.argv[2]

  # jsonlines version of europarl dataset (EN/NL)
  #pile_files = [open(pile_path + file_name + language + ".jsonl") for language in languages]

  # english version for now
  pile_files = [open(pile_path + file_name + "." + lang + ".jsonl")]
  ...
  try:
    ...
    fin = open("datasets/"+split+"_dataset_" + lang + ".csv")
  except:
  ...
  for i,row in enumerate(csvreader):
    ...
    # lets only consider long examples for now
    if (size >= 200 and fid == 0):
      print("long example found at index ", i)
      # CHANGE
      
      print(pile_files[fid].seek(line_byte_offset))
