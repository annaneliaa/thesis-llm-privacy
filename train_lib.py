...
def split_set_to_train_val(eval_percentage, output_dir, dataset_path):
    # Load the dataset
    with open(dataset_path, "r") as f:
        dataset = f.readlines()
    ...
    # Save train and eval datasets to files
    train_out_file = os.path.join(output_dir, "train.txt")
    with open(train_out_file, "w") as f:
        f.writelines(train_dataset)

    # Save train and eval datasets to files
    val_out_file = os.path.join(output_dir, "evaluation.txt")
    with open(val_out_file, "w") as f:
        f.writelines(eval_dataset)