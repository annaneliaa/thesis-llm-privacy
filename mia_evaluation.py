...
# Output: One dictionary of all dictionary entries in the list
...
# Writes some stas about the data in results to the file
...
# Evaluates results by writing statistics
def generate_stats(results: list, sentence_lengths: list, dir: str):
    file = os.path.join(dir, "stats.txt")
    write_stats(results, file)
    increased_perplexity_amt = 0
    # Analyze for what percentage of sentences the perplexity ratio is greater than 1
    ...
    # Gather and write stats for all results that are above the 75, 90, and 99 percentile
...
def evaluate_experiment():
    ...
    # Get the correct paths, and read the dataset
    ...
    # Load the losses and generate some stats about them
    ...
    # Get adn save the sentence lengths corresponding to all perplexity ratios
    ...
    # Generate stats for the results
    results_list = list(results.values())
    generate_stats(results_list, sentence_lengths, res_dir)
    ...
    for i, result in enumerate(results_list_dict):
        with open(stats_file, "a") as f:
            f.write(f"\n---- Stats for batch {i} ----\n")
        generate_stats(list(result.values()), sentence_lengths[prev:prev+len(result.values())], res_dir)
...
def evaluate_epochs(epochs: int):
    ...
    plot_means_medians(sorted(folders), dir, result_dir, f"{epochs} epoch(s) of training")
...
def main():
    # Choose the correct evaluation mode based on the eval_mode flag
    if not args.eval_mode:
        evaluate_experiment()
    elif args.eval_mode == "epoch":
